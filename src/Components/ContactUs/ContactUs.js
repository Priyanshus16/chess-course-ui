...
const ContactUs = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post(`${process.env.REACT_APP_BASE_URL}/send-email`, formData);
  ...
  return (
    <Container sx={{ mb: 8, display: "flex", flexDirection: "column", alignItems: "center" }}>
      <Typography variant="h3" gutterBottom sx={{ fontWeight: "bold", textAlign: "center", color: "#2E3B55", mt: 5 }}>
        Book Your Free Demo Class ♟️
      </Typography>
        ...
        <Grid item xs={12} md={6}>
          <Paper elevation={4} sx={{ p: 4, borderRadius: "10px" }}>
            ...
            <form onSubmit={handleSubmit}>
              <TextField fullWidth label="Your Name" name="name" variant="outlined" margin="normal" onChange={handleChange} required />
              <TextField fullWidth label="WhatsApp Contact" name="contact" variant="outlined" margin="normal" onChange={handleChange} required />
              <TextField fullWidth label="Email Address" name="email" variant="outlined" margin="normal" onChange={handleChange} required />
              <TextField fullWidth label="Your City" name="city" variant="outlined" margin="normal" onChange={handleChange} required />

              <Box sx={{ display: "flex", gap: 2 }}>
                <TextField fullWidth select label="Select Age Group" name="ageGroup" variant="outlined" margin="normal" onChange={handleChange} required>
                  <MenuItem value="5-7">5-7</MenuItem>
                  <MenuItem value="8-10">8-10</MenuItem>
                  <MenuItem value="11-15">11-15</MenuItem>
                </TextField>
                <TextField fullWidth select label="Preferred Language" name="language" variant="outlined" margin="normal" onChange={handleChange} required>
                  <MenuItem value="hindi">Hindi</MenuItem>
                  <MenuItem value="english">English</MenuItem>
                </TextField>
              </Box>

              <Button type="submit" variant="contained" color="success" sx={{ mt: 2, width: "100%", fontSize: "1rem", fontWeight: "bold", textTransform: "none" }}>
                ✅ Book Now
              </Button>
            </form>

            {status && <Typography variant="body2" sx={{ mt: 2, color: "red", textAlign: "center" }}>{status}</Typography>}
          </Paper>
        </Grid>
        <Grid item xs={12} md={6}>
          <Paper
