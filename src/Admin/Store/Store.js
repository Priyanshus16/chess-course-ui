...
export default function Store() {
  ...
  const getData = async () => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_BASE_ADMIN_URL}/store`
      );
  ...
  const handleSave = async (id) => {
    try {
      await axios.put(
        `${process.env.REACT_APP_BASE_ADMIN_URL}/store/${id}`,
        editedData
      );
      setApiData((prevData) =>
        prevData.map((item) =>
    ...
    >
      ...
      {selectedBlog && (
        <Dialog open={isModalOpen} onClose={handleCloseModal}>
          ...
          <DialogTitle>{selectedBlog.price}</DialogTitle>
          <DialogTitle>{selectedBlog.category}</DialogTitle>
          <DialogContent>
            <Typography>{selectedBlog.shortDescription}</Typography>
