...
//         `${process.env.REACT_APP_BASE_ADMIN_URL}/banner`
//       );
//       setApiData(response.data.banners);
//     } catch (error) {
...
//     try {
//       await axios.put(
//         `${process.env.REACT_APP_BASE_ADMIN_URL}/banner/${id}`,
//         editedData
//       );
//       setApiData((prevData) =>
//         prevData.map((item) =>
...
export default function Banner() {
  ...
  const getData = async () => {
    try {
      const response = await axios.get(`${process.env.REACT_APP_BASE_ADMIN_URL}/banner`);
      setApiData(response.data.banners || []);
    } catch (error) {
      console.error("Error fetching data:", error);
    }
  ...
  const handleSave = async (id) => {
    try {
      await axios.put(`${process.env.REACT_APP_BASE_ADMIN_URL}/banner/${id}`, editedData);
      setApiData((prevData) =>
  ...
  const handleUserDelete = async (id) => {
    ...
    if (result.isConfirmed) {
      try {
        await axios.delete(`${process.env.REACT_APP_BASE_ADMIN_URL}/banner/${id}`);
        setApiData((prevData) => prevData.filter((item) => item._id !== id));
