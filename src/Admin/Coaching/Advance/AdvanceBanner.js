...
// export default function AdvanceBanner() {
...
//         `${process.env.REACT_APP_BASE_ADMIN_URL}/advanceBanner`
//       );
//       setApiData(response.data.advanceBanner);
//     } catch (error) {
...
//     try {
//       await axios.put(
//         `${process.env.REACT_APP_BASE_ADMIN_URL}/advanceBanner/${id}`,
//         editedData
//       );
//       setApiData((prevData) =>
//         prevData.map((item) =>
...
//           }}
//         >
//           Advance Banner Management
//         </Typography>
//         <Button
//           variant="contained"
//           color="primary"
//           startIcon={<AddIcon />}
//           onClick={() => navigate("/admin/addAdvanceBanner")}
...
export default function AdvanceBanner() {
  ...
  const getData = async () => {
    try {
      const response = await axios.get(`${process.env.REACT_APP_BASE_ADMIN_URL}/advanceBanner`);
      setApiData(response.data.advanceBanner || []);
    } catch (error) {
      console.error("Error fetching data:", error);
    }
  ...
  const handleSave = async (id) => {
    try {
      await axios.put(`${process.env.REACT_APP_BASE_ADMIN_URL}/advanceBanner/${id}`, editedData);
      setApiData((prevData) =>
  ...
  const handleUserDelete = async (id) => {
    ...
    if (result.isConfirmed) {
      try {
        await axios.delete(`${process.env.REACT_APP_BASE_ADMIN_URL}/advanceBanner/${id}`);
        setApiData((prevData) => prevData.filter((item) => item._id !== id));
        Swal.fire("Deleted!", "Your advance banner has been deleted.", "success");
      } catch (error) {
        console.error("Error deleting banner:", error);
        Swal.fire("Error!", "Failed to delete advance banner.", "error");
      }
    }
  };
    ...
    >
      ...
      <Box sx={{ mb: 2 }}>
        <TextField
          fullWidth
          variant="outlined"
          placeholder="Search advance banners..."
          value={searchTerm}
          onChange={(e) => setSearchTerm(e.target.value)}
          InputProps={{
            startAdornment: (
