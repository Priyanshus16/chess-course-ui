...
// export default function BeginnerBenefit() {
...
//         `${process.env.REACT_APP_BASE_ADMIN_URL}/beginnerBenefit`
//       );
//       console.log(response);
//       setApiData(response.data.beginnerCard);
//     } catch (error) {
...
//     try {
//       await axios.put(
//         `${process.env.REACT_APP_BASE_ADMIN_URL}/beginnerBenefit/${id}`,
//         editedData
//       );
//       setApiData((prevData) =>
//         prevData.map((item) =>
...
//           color="primary"
//           startIcon={<AddIcon />}
//           onClick={() => navigate("/admin/addBeginnerBenefit")}
//         >
//           Create
//         </Button>
//       </Box>
...
export default function BeginnerBenefit() {
  ...
  const getData = async () => {
    try {
      const response = await axios.get(`${process.env.REACT_APP_BASE_ADMIN_URL}/beginnerBenefit`);
      setApiData(response.data.beginnerCard || []);
    } catch (error) {
      console.error("Error fetching data:", error);
    }
  ...
  const handleSave = async (id) => {
    try {
      await axios.put(`${process.env.REACT_APP_BASE_ADMIN_URL}/beginnerBenefit/${id}`, editedData);
      setApiData((prevData) =>
        prevData.map((item) => (item._id === id ? { ...item, ...editedData } : item))
      );
      setEditId(null);
    } catch (error) {
      console.error("Error updating beginner benefit:", error);
    }
  };
  ...
  const handleUserDelete = async (id) => {
    ...
    if (result.isConfirmed) {
      try {
        await axios.delete(`${process.env.REACT_APP_BASE_ADMIN_URL}/beginnerBenefit/${id}`);
        setApiData((prevData) => prevData.filter((item) => item._id !== id));
        Swal.fire("Deleted!", "Your beginner benefit card has been deleted.", "success");
      } catch (error) {
        console.error("Error deleting beginner benefit:", error);
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          Beginner Benefit Cards Management
        </Typography>
        <Button
          variant="contained"
          color="primary"
