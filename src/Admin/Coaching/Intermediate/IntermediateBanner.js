...
// export default function IntermediateBanner() {
...
//         `${process.env.REACT_APP_BASE_ADMIN_URL}/IntermediateBanner`
//       );
//       setApiData(response.data.intermediateBanner);
//     } catch (error) {
...
//     try {
//       await axios.put(
//         `${process.env.REACT_APP_BASE_ADMIN_URL}/IntermediateBanner/${id}`,
//         editedData
//       );
//       setApiData((prevData) =>
//         prevData.map((item) =>
...
//   const filteredBlogs = apiData.filter((banner) =>
...
//           }}
//         >
//           Intermediate Banner Management
//         </Typography>
//         <Button
//           variant="contained"
//           color="primary"
//           startIcon={<AddIcon />}
//           onClick={() => navigate("/admin/addIntermediateBanner")}
//           disabled={apiData.length >= 1}
//         >
//           Create
//         </Button>
//       </Box>
...
export default function IntermediateBanner() {
  ...
  const [searchTerm, setSearchTerm] = useState("");
  const [selectedBanner, setSelectedBanner] = useState(null); // Renamed for clarity
  const [isModalOpen, setIsModalOpen] = useState(false);
  ...
  const getData = async () => {
    try {
      const response = await axios.get(`${process.env.REACT_APP_BASE_ADMIN_URL}/IntermediateBanner`);
      setApiData(response.data.intermediateBanner || []);
    } catch (error) {
      console.error("Error fetching data:", error);
    }
  ...
  const handleSave = async (id) => {
    try {
      await axios.put(`${process.env.REACT_APP_BASE_ADMIN_URL}/IntermediateBanner/${id}`, editedData);
      setApiData((prevData) =>
  ...
  const handleUserDelete = async (id) => {
    ...
    if (result.isConfirmed) {
      try {
        await axios.delete(`${process.env.REACT_APP_BASE_ADMIN_URL}/IntermediateBanner/${id}`);
        setApiData((prevData) => prevData.filter((item) => item._id !== id));
        Swal.fire("Deleted!", "Your intermediate banner has been deleted.", "success");
      } catch (error) {
        console.error("Error deleting banner:", error);
        Swal.fire("Error!", "Failed to delete intermediate banner.", "error");
      }
    }
  };
  ...
  const filteredBanners = apiData.filter((banner) =>
    banner.heading.toLowerCase().includes(searchTerm.toLowerCase())
  );
    ...
    >
      ...
      <Box sx={{ mb: 2 }}>
        <TextField
          fullWidth
          variant="outlined"
          placeholder="Search intermediate banners..."
          value={searchTerm}
          onChange={(e) => setSearchTerm(e.target.value)}
          InputProps={{
            startAdornment: (
