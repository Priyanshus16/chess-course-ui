...
// export default function IntermediateBenefit() {
...
//         `${process.env.REACT_APP_BASE_ADMIN_URL}/intermediateBenefit`
//       );
//       console.log(response);
//       setApiData(response.data.intermediateCard);
//     } catch (error) {
...
//     try {
//       await axios.put(
//         `${process.env.REACT_APP_BASE_ADMIN_URL}/intermediateBenefit/${id}`,
//         editedData
//       );
//       setApiData((prevData) =>
//         prevData.map((item) =>
...
//           color="primary"
//           startIcon={<AddIcon />}
//           onClick={() => navigate("/admin/addIntermediateBenefit")}
//         >
//           Create
//         </Button>
//       </Box>
...
export default function IntermediateBenefit() {
  ...
  const getData = async () => {
    try {
      const response = await axios.get(`${process.env.REACT_APP_BASE_ADMIN_URL}/intermediateBenefit`);
      setApiData(response.data.intermediateCard || []);
    } catch (error) {
      console.error("Error fetching data:", error);
    }
  ...
  const handleSave = async (id) => {
    try {
      await axios.put(`${process.env.REACT_APP_BASE_ADMIN_URL}/intermediateBenefit/${id}`, editedData);
      setApiData((prevData) =>
        prevData.map((item) => (item._id === id ? { ...item, ...editedData } : item))
      );
      setEditId(null);
    } catch (error) {
      console.error("Error updating intermediate benefit:", error);
    }
  };
  ...
  const handleUserDelete = async (id) => {
    ...
    if (result.isConfirmed) {
      try {
        await axios.delete(`${process.env.REACT_APP_BASE_ADMIN_URL}/intermediateBenefit/${id}`);
        setApiData((prevData) => prevData.filter((item) => item._id !== id));
        Swal.fire("Deleted!", "Your intermediate benefit card has been deleted.", "success");
      } catch (error) {
        console.error("Error deleting intermediate benefit:", error);
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          Intermediate Benefit Cards Management
        </Typography>
        <Button
          variant="contained"
          color="primary"
